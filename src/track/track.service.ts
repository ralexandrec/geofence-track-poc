...
export class TrackService {
  ...
  findAll() {
    return this.tracks.map((t) => this.formatTrack(t));
  }

  findOne(id: number) {
    const found = this.tracks.find((track) => track.id === id) ?? null;
    return found ? this.formatTrack(found) : null;
  }
  ...
  remove(id: number) {
    ...
    return this.formatTrack(removed);
  }
}
